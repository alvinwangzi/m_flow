...
- [m_flow/pipeline/operations/dispatch_remote.py](file://m_flow/pipeline/operations/dispatch_remote.py)
...
## 更新摘要
**所做更改**
- 更新了执行与遥测包装章节，增加了运行完成和失败时自动触发进度更新的机制
- 新增了进度跟踪与资源使用概览章节的详细说明
- 更新了故障排查指南，增加了进度更新相关的排查要点
- 增强了监控架构图，展示了自动进度更新的完整流程
...
D["run_tasks<br/>任务执行"]
E["dispatch_remote<br/>远程调度"]
end
subgraph "事件与记录"
F["RunEvent / RunStarted / RunCompleted / RunFailed<br/>运行事件"]
G["WorkflowRun<br/>数据库记录"]
H["record_run_finish<br/>完成记录"]
I["record_run_failure<br/>失败记录"]
end
subgraph "日志与遥测"
J["logging_utils.setup_logging<br/>日志初始化"]
K["utils.send_telemetry<br/>遥测上报"]
end
subgraph "追踪系统"
L["tracing.TraceManager<br/>追踪管理"]
M["tracing.JsonlTraceSink / MemoryRingTraceSink<br/>追踪输出"]
end
A --> B
B --> F
B --> K
...
- [m_flow/pipeline/operations/run_tasks.py:220-239](file://m_flow/pipeline/operations/run_tasks.py#L220-L239)
...
- 进度更新 update_pipeline_progress：在 STARTED 记录中以 JSON 字段维护 total_items、processed_items、current_step、时间戳，支持自动触发
...
## 架构总览
下图展示一次管道执行的监控与调试路径：从执行入口到事件记录、遥测、日志与追踪，以及自动进度更新机制。
...
participant Tasks as "run_tasks"
participant Remote as "dispatch_remote"
participant Finish as "record_run_finish"
participant Fail as "record_run_failure"
...
Note over Progress,DB : 自动触发进度更新
Tasks->>Finish : 成功完成时
Finish->>Progress : 更新 current_step="Completed"
Fail->>Progress : 更新 current_step="Failed"
Progress->>DB : 更新 run_detail.progress
...
- **新增**：自动进度更新机制：在成功完成和失败时自动调用 update_pipeline_progress 更新 current_step 状态
...
Wrap->>Progress : 更新 current_step="Completed"
else 异常
Wrap->>Tele : 发送 "Pipeline Run Errored"
Wrap->>DB : 记录 ERRORED
Wrap->>Progress : 更新 current_step="Failed"
end
```
...
- **新增**：自动触发机制：在 record_run_finish 和 record_run_failure 中自动调用 update_pipeline_progress 更新最终状态
- **增强**：在远程调度中，每个批次完成后都会调用 update_pipeline_progress 更新 processed_items
...
**章节来源**
- [m_flow/pipeline/operations/update_pipeline_progress.py:27-159](file://m_flow/pipeline/operations/update_pipeline_progress.py#L27-L159)
- [m_flow/pipeline/operations/record_run_finish.py:55](file://m_flow/pipeline/operations/record_run_finish.py#L55)
- [m_flow/pipeline/operations/record_run_failure.py:62](file://m_flow/pipeline/operations/record_run_failure.py#L62)
- [m_flow/pipeline/operations/dispatch_remote.py:172](file://m_flow/pipeline/operations/dispatch_remote.py#L172)
...
- **新增**：record_run_finish 和 record_run_failure 依赖 update_pipeline_progress 实现自动进度更新
- **新增**：dispatch_remote 在远程执行过程中依赖 update_pipeline_progress 实现实时进度更新
...
R --> PR["PipelineRun 模型"]
PR --> DB["关系型引擎"]
R --> UP["update_pipeline_progress"]
DR["dispatch_remote"] --> UP
UP --> PR
...
- [m_flow/pipeline/operations/execute_with_telemetry.py:24-90](file://m_flow/pipeline/operations/execute_with_telemetry.py#L24-L90)
- [m_flow/pipeline/models/PipelineRun.py:36-52](file://m_flow/pipeline/models/PipelineRun.py#L36-L52)
- [m_flow/pipeline/operations/record_run_finish.py:15](file://m_flow/pipeline/operations/record_run_finish.py#L15)
- [m_flow/pipeline/operations/record_run_failure.py:15](file://m_flow/pipeline/operations/record_run_failure.py#L15)
- [m_flow/pipeline/operations/dispatch_remote.py:172](file://m_flow/pipeline/operations/dispatch_remote.py#L172)
- [m_flow/shared/tracing/sinks.py:34-110](file://m_flow/shared/tracing/sinks.py#L34-L110)
...
- **新增**：自动进度更新优化：在远程执行中每批次完成后才更新进度，减少数据库写入频率
...
- 常见问题
  - 进度未更新：检查是否处于 STARTED 记录且 run_detail.progress 是否存在；确认 determinism 场景下 ORDER BY created_at DESC LIMIT 1 生效；验证自动进度更新是否正常触发
  - 遥测未上报：检查 TELEMETRY_DISABLED/ENV=test|dev；确认端点与网络连通
  - 追踪未落盘：检查 MFLOW_TRACE_ENABLED 与 MFLOW_TRACE_DIR；内存环仅用于调试
  - **新增**：完成/失败状态异常：检查 record_run_finish 和 record_run_failure 是否正确调用了 update_pipeline_progress
- 复盘步骤
...
## 结论
m_flow 的管道监控与调试体系以"事件 + 记录 + 遥测 + 日志 + 追踪"为核心，既保证了运行时可观测性，又兼顾了生产环境的性能与稳定性。通过 RunEvent 与 WorkflowRun 的组合，可完整回溯执行历史；通过 execute_with_telemetry 与 update_pipeline_progress，可实现进度与状态的实时掌控；借助 logging_utils 与 tracing，可快速定位问题并进行可视化复盘。

**重要改进**：最新的自动进度更新机制确保了工作流完成和失败状态能够正确反映在进度跟踪系统中，通过 record_run_finish 和 record_run_failure 自动调用 update_pipeline_progress，实现了从启动到完成/失败的完整进度跟踪闭环。

建议在生产环境中结合外部 APM 与告警策略，形成闭环监控。
...
- 案例二：部分批次失败
  ...
  - 行为：run_detail 中包含 error 与 data 摘要，并自动更新进度状态为"Failed"
  - 排查：查看日志异常钩子与追踪文件，定位具体批次与阶段
- 案例三：进度停滞
  - 触发点：update_pipeline_progress 未更新或未找到 STARTED 记录
  - 行为：检查 determinism 与 run_id 生成逻辑；确认自动进度更新是否正常触发
  - 排查：确认 ORDER BY created_at DESC LIMIT 1 生效；检查进度更新日志
- **新增**：自动进度更新失效
  - 触发点：工作流完成但进度状态未更新
  - 行为：检查 record_run_finish 是否正确调用 update_pipeline_progress
  - 排查：查看进度更新日志，确认异常处理是否正常记录
